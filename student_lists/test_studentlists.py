...
"""
# assertIn/NotIn- check to see if value is in list or substring
...
class TestStudentLists(TestCase):
    ...
    def test_cant_create_calls_with_negative_students(self):
        with self.assertRaises(StudentError):
            test_class = ClassList(-1)


    def test_cant_create_calls_with_zero_students(self):
        with self.assertRaises(StudentError):
            test_class = ClassList(0)


    def test_can_create_class_with_positive_number_of_students(self):
        test_class = ClassList(1)  # what do you think about this test? Should it be here? Can it be improved?
    ...
    def test_add_example_students(self):
        ...
        test_class.add_student('Another Test Student')
        with self.assertRaises(StudentError):
            test_class.remove_student('Yet Another')
    ...
    def test_student_not_enrolled_in_test_class(self):
        test_class = ClassList(2)
        test_class.add_student('Snoop Dogg')
        test_class.add_student('Martha Stewart')
        self.assertFalse(test_class.is_enrolled('Beyonce Knowles'))
    ...
    def test_index_of_students_is_none_if_classList_not_empty(self):
        test_class = ClassList(2)
        test_class.add_student('Test Student')
        test_class.add_student('Hermione')
        index = test_class.index_of_student('Ron')
        self.assertIsNone(index)
    ...
    def test_class_is_not_full_or_it_is_empty(self):
        test_class = ClassList(2)
        test_class.add_student('Test Student')
        self.assertFalse(test_class.class_is_full)


# can also run unit test from terminal using python -m unittest test_studentlists.py